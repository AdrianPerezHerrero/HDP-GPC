...
try:
    import hdbscan as hdbscan_external
except Exception:  # pragma: no cover - optional dependency
    hdbscan_external = None
...
def safe_silhouette(X: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    try:
        X_arr = np.asarray(X)
        if X_arr.ndim == 2 and X_arr.shape[0] == X_arr.shape[1] and X_arr.shape[0] == len(y_pred):
            return float(silhouette_score(X_arr, y_pred, metric="precomputed"))
        return float(silhouette_score(X_arr, y_pred))
    except Exception:
        return float("nan")
...
def dtw_distance_1d(a: np.ndarray, b: np.ndarray, window: Optional[int] = None) -> float:
    """
    Basic DTW distance for two 1D sequences.

    window is an optional Sakoe-Chiba half-window. If None, unrestricted DTW is used.
    """
...
def compute_dtw_distance_matrix(
    ...
    """
    Compute a symmetric DTW distance matrix.

    DTW baselines are O(N^2), so max_samples can be used to select a reproducible
    subset for quick tests or very large records.
    """
    ...
    n_use = X_use.shape[0]
    D = np.zeros((n_use, n_use), dtype=np.float32)
    for i in range(n_use):
        if i % 100 == 0 and n_use > 250:
            print(f"    [DTW] distances row {i}/{n_use}")
        xi = X_use[i]
        for j in range(i + 1, n_use):
            d = dtw_distance_1d(xi, X_use[j], window=window)
            D[i, j] = D[j, i] = d
    return D, idx
...
def kmedoids_precomputed(
    ...
    """Lightweight PAM-style k-medoids using a precomputed distance matrix."""
    D = np.asarray(D, dtype=np.float64)
    n = D.shape[0]
    k = int(min(max(1, n_clusters), n))
    rng = np.random.default_rng(seed)
    ...
    for _ in range(max_iter):
        ...
        for c in range(k):
            ...
            medoids[c] = members[np.argmin(within.sum(axis=1))]
        labels = np.argmin(D[:, medoids], axis=1)
...
def _agglomerative_precomputed(D: np.ndarray, n_clusters: int, linkage: str) -> np.ndarray:
    """Version-safe wrapper for sklearn AgglomerativeClustering with precomputed distances."""
    kwargs = dict(n_clusters=int(n_clusters), linkage=linkage)
    try:
...
def run_dtw_hierarchical(D: np.ndarray, n_clusters: int, linkage: str = "average") -> np.ndarray:
    if linkage == "ward":
        raise ValueError("Ward linkage is not compatible with precomputed DTW distances.")
    return _agglomerative_precomputed(D, n_clusters=n_clusters, linkage=linkage)


def choose_hdbscan_precomputed(
    D: np.ndarray,
    min_cluster_size_grid: List[int],
    min_samples_grid: List[int],
) -> Tuple[np.ndarray, Dict[str, float]]:
    """
    Select HDBSCAN parameters on a precomputed distance matrix by silhouette over non-noise points.
    Requires either the external `hdbscan` package or sklearn.cluster.HDBSCAN.
    """
    best_labels: Optional[np.ndarray] = None
    best_info: Dict[str, float] = {
        ...
        "n_clusters_no_noise": np.nan,
    }

    if hdbscan_external is None and SklearnHDBSCAN is None:
        raise ImportError(
            "HDBSCAN is not available. Install `hdbscan` or use a scikit-learn version "
            "that provides sklearn.cluster.HDBSCAN."
        )

    n = D.shape[0]
    for min_cluster_size in min_cluster_size_grid:
        if min_cluster_size < 2 or min_cluster_size >= n:
            continue
        for min_samples in min_samples_grid:
            if min_samples < 1 or min_samples >= n:
                continue
            try:
                if hdbscan_external is not None:
                    model = hdbscan_external.HDBSCAN(
                        metric="precomputed",
            ...
            try:
                score = silhouette_score(D[np.ix_(mask, mask)], labels[mask], metric="precomputed")
            except Exception:
                continue
    ...
    return best_labels.astype(np.int64), best_info


def classical_k_from_args(args: argparse.Namespace, y: np.ndarray, D: np.ndarray, seed: int) -> Tuple[int, float]:
    if args.classical_k == "oracle":
        return int(len(np.unique(y))), float("nan")
    if args.classical_k == "auto":
        best_k = 2
        best_s = -np.inf
        k_max = min(args.max_classical_k, max(2, len(y) - 1))
        for k in range(2, k_max + 1):
            try:
                pred = _agglomerative_precomputed(D, n_clusters=k, linkage=args.dtw_hier_linkage)
                s = silhouette_score(D, pred, metric="precomputed")
            except Exception:
                continue
            if s > best_s:
                best_k, best_s = int(k), float(s)
        if not np.isfinite(best_s):
            best_s = float("nan")
        return best_k, best_s
    return int(args.classical_k), float("nan")


def subset_record_for_classical(record: RecordData, idx: np.ndarray) -> RecordData:
    return RecordData(rec=record.rec, X=record.X[idx], y=record.y[idx])
...
def run_record(record: RecordData, args: argparse.Namespace, device: torch.device, out_dir: Path) -> List[Dict[str, object]]:
    ...
    deep_methods = {"cae-dpgmm", "cae-dbscan", "dec"}
    classical_methods = {"dtw-kmedoids", "dtw-hierarchical", "dtw-hdbscan"}
    requested_deep = any(m in args.methods for m in deep_methods)
    requested_classical = any(m in args.methods for m in classical_methods)

    latent_scaled: Optional[np.ndarray] = None
    latent: Optional[np.ndarray] = None
    ae: Optional[ConvAutoencoder1D] = None
    ae_minutes = 0.0

    if requested_classical:
        t_dtw = time.time()
        D, idx = compute_dtw_distance_matrix(
            record.X,
            window=args.dtw_window,
            max_samples=args.dtw_max_samples,
            seed=args.seed,
        )
        dtw_minutes = (time.time() - t_dtw) / 60.0
        classical_record = subset_record_for_classical(record, idx)
        classical_k, classical_k_sil = classical_k_from_args(args, classical_record.y, D, args.seed)

        if "dtw-kmedoids" in args.methods:
            t = time.time()
            pred = kmedoids_precomputed(D, n_clusters=classical_k, seed=args.seed, max_iter=args.kmedoids_max_iter)
            save_labels(base / "dtw_kmedoids" / f"cluster_labels_{record.rec}.npy", pred)
            rows.append({
                "record": record.rec,
                "method": "DTW+k-medoids",
                "fit_minutes": dtw_minutes + (time.time() - t) / 60.0,
                "classical_k": classical_k,
                "classical_auto_k_silhouette": classical_k_sil,
                "dtw_subset_size": len(classical_record.y),
                **score_clustering(classical_record.y, pred, D),
            })

        if "dtw-hierarchical" in args.methods:
            t = time.time()
            pred = run_dtw_hierarchical(D, n_clusters=classical_k, linkage=args.dtw_hier_linkage)
            save_labels(base / "dtw_hierarchical" / f"cluster_labels_{record.rec}.npy", pred)
            rows.append({
                "record": record.rec,
                "method": f"DTW+hierarchical-{args.dtw_hier_linkage}",
                "fit_minutes": dtw_minutes + (time.time() - t) / 60.0,
                "classical_k": classical_k,
                "classical_auto_k_silhouette": classical_k_sil,
                "dtw_subset_size": len(classical_record.y),
        ...
        if "dtw-hdbscan" in args.methods:
            t = time.time()
            pred, info = choose_hdbscan_precomputed(
                D,
                min_cluster_size_grid=args.hdbscan_min_cluster_size,
                min_samples_grid=args.hdbscan_min_samples,
            )
            save_labels(base / "dtw_hdbscan" / f"cluster_labels_{record.rec}.npy", pred)
            rows.append({
                "record": record.rec,
                "method": "DTW+HDBSCAN",
                "fit_minutes": dtw_minutes + (time.time() - t) / 60.0,
                "dtw_subset_size": len(classical_record.y),
                **score_clustering(classical_record.y, pred, D),
                **{f"hdbscan_{k}": v for k, v in info.items()},
            })
    ...
    if "cae-dpgmm" in args.methods:
        assert latent is not None and latent_scaled is not None
        t = time.time()
    ...
    if "cae-dbscan" in args.methods:
        assert latent_scaled is not None
        t = time.time()
    ...
    if "dec" in args.methods:
        assert latent_scaled is not None and ae is not None
        t = time.time()
        if args.dec_k == "auto":
    ...
    if args.include_hdp:
        ...
        if hdp is not None and len(hdp) == len(record.y):
            score_embed = latent_scaled if latent_scaled is not None else record.X
            rows.append({"record": record.rec, "method": "HDP-GPC", "fit_minutes": float("nan"), **score_clustering(record.y, hdp, score_embed)})
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--methods",
        nargs="+",
        default=["cae-dpgmm", "cae-dbscan", "dec", "dtw-kmedoids", "dtw-hierarchical", "dtw-hdbscan"],
        choices=[
            "cae-dpgmm",
            "cae-dbscan",
            "dec",
    ...
    p.add_argument("--classical-k", type=str, default="auto", help="oracle | auto | integer for DTW+k-medoids and DTW+hierarchical")
    p.add_argument("--max-classical-k", type=int, default=20)
    p.add_argument("--dtw-window", type=int, default=None, help="Optional Sakoe-Chiba DTW half-window. Default: unrestricted.")
    p.add_argument("--dtw-max-samples", type=int, default=None, help="Optional maximum number of beats for O(N^2) DTW baselines.")
    p.add_argument("--dtw-hier-linkage", type=str, default="average", choices=["average", "complete", "single"])
    p.add_argument("--kmedoids-max-iter", type=int, default=100)
    p.add_argument("--hdbscan-min-cluster-size", nargs="+", type=int, default=[5, 10, 20])
    p.add_argument("--hdbscan-min-samples", nargs="+", type=int, default=[3, 5, 10])
