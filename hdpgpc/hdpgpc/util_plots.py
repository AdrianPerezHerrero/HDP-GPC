...
def print_results(sw_gp, labels, N_0, error=False, purity=False):
    ...
    print(f"Classification error: {int(err.sum())} / {sw_gp.T} -- {(int(err.sum()) / sw_gp.T):5.f}")
    if purity:
        print(f"Classification purity: {sw_gp.T - int(err.sum())}/{sw_gp.T} -- {(1 - err.sum() / sw_gp.T):5.f}")
    if purity:
