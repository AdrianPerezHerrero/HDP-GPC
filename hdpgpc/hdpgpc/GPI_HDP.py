...
class GPI_HDP():
    ...
    def redefine_default(self, x_trains, y_trains, resp):
        ...
        ini_Gamma = self.cond_to_torch(np.min([np.max([var_y_y_,var_y_y * 1.0]), var_y_y * 2.0])) * 0.2
    ...
    def include_sample(self, x_train, y, with_warp=True, force_model=None, minibatch=0, classify=False):
        ...
        if t > 0:
            ...
            for ld in range(self.n_outputs):
                ...
                #rov_gp.bayesian_new_params(1.0)
            ...
            elbo_prev_post = elbo_prev_post / self.T
            for ld in range(self.n_outputs):
            ...
            if torch.argmax(q_prev[-1]) == self.M:
                ...
                for m in q_ord:
                    ...
                    elbo_bas_post = elbo_bas_post / self.T
