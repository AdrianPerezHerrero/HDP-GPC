...
class GPI_HDP():
        ...
        while True:
            ...
            if self.T > 1:
                ...
                if ((np.isclose(elbo, elbo_, rtol=1e-5) and resp_group[-1] < 1.0) or
                        torch.where(resp_group==0.0)[0].shape[0] > 1.0 or
    ...
    def redefine_default(self, x_trains, y_trains, resp):
        ...
        var_y_y_ = torch.median(torch.diag(gp.Gamma[ind_]))
        var_y_y = torch.median(torch.diag(gp.Sigma[ind_]))
        # samples = y_trains[:n_f][:, :, 0].T
        # samples_ = y_trains[1:n_f+1][:, :, 0].T
        ...
        #ini_Gamma = self.cond_to_torch(np.min([np.max([var_y_y_,var_y_y * 1.0]), var_y_y * 2.5])) * 0.012
        ...
        bound_sigma = (ini_Sigma.item() * 1e-5, 0.1)
        bound_gamma = (ini_Gamma.item() * 1e-5, 0.1)
