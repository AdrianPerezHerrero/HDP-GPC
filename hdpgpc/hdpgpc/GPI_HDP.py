...
class GPI_HDP():
    ...
    def estimate_q_first(self, M, x_trains, y_trains, y_trains_w, resp, respPair, q_, q_lat_, snr_, startPi, transPi, reallocate_=False, reparam=False):
        ...
        f_ind_new_potential = torch.argsort(self.weight_mean(q_ + q_lat)[torch.where(resp == 1.0)])
    ...
    def redefine_default(self, x_trains, y_trains, resp):
        ...
        ini_Gamma = self.cond_to_torch(np.min([np.max([var_y_y_,var_y_y * 1.2]), var_y_y * 2.5])) * 0.3
        #ini_Sigma = var_y_y * 2.0
        #ini_Gamma = self.cond_to_torch(np.min([np.max([var_y_y_,var_y_y * 1.0]), var_y_y * 2.5])) * 2.0
        #ini_Gamma = var_y_y_ * 1.0
        #ini_Sigma = 50.0
        #ini_Gamma = 100.0
        ini_Sigma = self.cond_to_torch(np.min([ini_Sigma, 250.0]))
        ini_Gamma = self.cond_to_torch(np.min(([ini_Gamma, 300.0])))
    ...
    def include_sample(self, x_train, y, with_warp=True, force_model=None, minibatch=0, classify=False):
        ...
        if t > 0:
            ...
            for ld in range(self.n_outputs):
                ...
                q_lat_prev[:, -1, ld] = prov_gp.compute_q_lat_all(torch.from_numpy(np.array(self.x_train)), h_ini=0.1)
            ...
            if torch.argmax(q_prev[-1]) == self.M:
                ...
                for m in q_ord:
                    ...
                    for ld in range(self.n_outputs):
                        ...
                        q_lat_post[:, m, ld] = post_gp.compute_q_lat_all(torch.from_numpy(np.array(self.x_train)), h_ini=0.1)
                        #q_post[[-1], m, ld] = self.estimate_new(t, post_gp, self.x_train[-1], y_mod[m][-1], h=1.0)
        ...
        if birth:
            ...
            q_aux = torch.zeros(self.T, self.M + 1, self.n_outputs)
            q_aux[:,:-1,:] = q_chos
            resp, resplog, respPair, respPairlog = self.variational_local_terms(q_aux, self.transTheta,
