...
class GPI_HDP():
    ...
    def redefine_default(self, x_trains, y_trains, resp):
        ...
        ini_Gamma = self.cond_to_torch(np.max([var_y_y_,var_y_y * 1.1])) * 1.0
        bound_sigma = (ini_Sigma * 0.005, ini_Sigma * 1.0)
        bound_gamma = (ini_Gamma * 0.005, ini_Gamma * 1.0)
    ...
    def include_sample(self, x_train, y, with_warp=True, force_model=None, minibatch=0, classify=False):
        ...
        if t > 0:
            ...
            for ld in range(self.n_outputs):
                ...
                q_prev[[-1], -1, ld] = self.estimate_new(t, prov_gp, self.x_train[-1], y_mod[-1][-1])
                prov_gp.include_weighted_sample(t, self.x_train[-1], self.x_train[-1], y_mod[-1][-1], 1.0)
    ...
    def estimate_new(self, t, gpmodel, x_train, y):
        """
