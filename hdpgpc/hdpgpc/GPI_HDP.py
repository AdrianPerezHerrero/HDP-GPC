...
class GPI_HDP():
    ...
    def redefine_default(self, x_trains, y_trains, resp):
        ...
        ini_Sigma = var_y_y * 0.008
        #ini_Gamma = var_y_y_ * 0.2
        ini_Gamma = self.cond_to_torch(np.min([np.max([var_y_y_,var_y_y * 1.3]), var_y_y * 3.0])) * 0.008
        ...
        ini_Gamma = self.cond_to_torch(np.max(([ini_Gamma, 18.0])))
        #ini_Gamma = self.cond_to_torch(np.max([var_y_y_, var_y_y * 1.5]))
        #ini_Gamma = var_y_y_ * 1.5
        bound_sigma = (ini_Sigma * 1e-3, ini_Sigma * 1.0)
