...
class GPI_HDP():
    ...
    def include_batch(self, x_trains, y_trains, with_warp=False, force_model=None, it_limit=None, warp=False):
        ...
        while True:
            ...
            if self.T > 1:
                ...
                self.train_elbo.append(elbo_)
                self.resp_assigned.append(torch.where(resp == 1.0)[1])
                if ((np.isclose(elbo, elbo_, rtol=1e-5)) or
    ...
    def redefine_default(self, x_trains, y_trains, resp):
        ...
        #ini_Gamma = self.cond_to_torch(np.min([np.max([var_y_y_,var_y_y * 1.0]), var_y_y * 2.5])) * 0.060
        ...
        ini_Sigma = self.cond_to_torch(3.0)
        ini_Gamma = self.cond_to_torch(5.0)
        # if ini_Sigma > 200.0:
