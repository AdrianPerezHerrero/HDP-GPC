...
class GPI_HDP():
    ...
    def redefine_default(self, x_trains, y_trains, resp):
        ...
        ini_Sigma = var_y_y * 0.25
        ini_Gamma = self.cond_to_torch(np.min([np.max([var_y_y_, var_y_y * 1.1]), var_y_y * 1.5])) * 0.25
        #ini_Sigma = self.cond_to_torch(np.min([ini_Sigma, 45.0]))
        #ini_Gamma = self.cond_to_torch(np.min(([ini_Gamma, 60.0])))
        #ini_Gamma = self.cond_to_torch(np.max([var_y_y_, var_y_y * 1.5]))
        #ini_Gamma = var_y_y_ * 1.5
        bound_sigma = (np.sqrt(ini_Sigma) * 0.05, np.sqrt(ini_Sigma) * 1.0)
        bound_gamma = (np.sqrt(ini_Gamma) * 0.05, np.sqrt(ini_Gamma) * 1.0)
