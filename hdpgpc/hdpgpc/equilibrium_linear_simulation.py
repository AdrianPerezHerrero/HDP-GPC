...
No warping is used. The latent flow is observed through a configurable linear projection/mixing matrix.
...
def count_assigned_clusters(labels):
    """
    Number of clusters that actually receive at least one MAP-assigned sample.
    This can be smaller than the number of active BayesianGaussianMixture
    components by posterior weight.
    """
...
def make_linear_observation_matrix(
    latent_dim,
    n_outputs,
    seed=0,
    ...
    """
    Create a fixed linear observation/projection matrix W.

    The latent dynamical state z(t) lives in R^latent_dim and the observed signal is:

        y(t) = W z(t)

    If n_outputs > latent_dim, this is an embedding into a higher-dimensional
    observation space. If n_outputs < latent_dim, this is a dimensionality
    reduction / projection.

    The matrix is constructed with controlled singular values so the projection
    is not numerically degenerate.
    """
    rng = np.random.default_rng(seed)
    ...
    # Normalize so the average output scale is comparable across dimensions.
    W = W / np.sqrt(np.mean(W**2) * latent_dim + 1e-12)

    return W


def apply_linear_observation_map(Z, W):
    """
    Apply the linear observation model:

        Y = Z W^T

    where Z has shape [T, latent_dim] and W has shape [n_outputs, latent_dim].
    """
    return np.asarray(Z, dtype=np.float64) @ np.asarray(W, dtype=np.float64).T
...
def simulate_equilibrium_flows(
    ...
    # Latent dynamics live in R^latent_dim.
    v1, v2 = orthonormal_directions(latent_dim, seed=seed + 100)

    # Observations are a linear projection/mixing of the latent state.
    ...
    # Two latent equilibria are separated mostly on v1, with a small v2 component so
    # PCA displays smooth/non-collinear movement rather than a trivial line.
    ...
    for system in [0, 1]:
        ...
        for _ in range(n_traj_per_system):
            ...
            for t in range(T):
                ...
                deterministic = eq + (rho ** t) * (z0 - eq)

                # Mild autocorrelated perturbation around the latent path.
                if t == 0:
                    drift = rng.normal(0.0, drift_noise, size=latent_dim)
                else:
                    drift = 0.85 * drift + rng.normal(0.0, drift_noise, size=latent_dim)

                z_t = amp * deterministic + drift
                Y.append(z_t)

                y_prev = z_t

            Z = np.asarray(Y, dtype=np.float64)
            Y_obs = apply_linear_observation_map(Z, W_obs)
            Y_obs = output_offset[None, :] + Y_obs
            Y_obs = Y_obs + rng.normal(0.0, obs_noise, size=Y_obs.shape)
    ...
    return data, labels, metadata, e0, e1, start_center, W_obs
...
def run_cloud_level_dpgmm(data, labels, out_dir, args):
    ...
    save_pooled_pca_point_clusters(
        ...
        f"Cloud-level DP-GMM: {count_assigned_clusters(pred)} assigned, {active} active-by-weight snapshot components",
    )
    ...
    return {
        ...
        "active_or_inferred_clusters": count_assigned_clusters(pred),
        "assigned_clusters": count_assigned_clusters(pred),
...
def run_trajectory_level_dpgmm(data, labels, out_dir, args):
    ...
    save_pooled_pca_trajectory_clusters(
        ...
        f"Trajectory-level static DP-GMM paths: {count_assigned_clusters(pred)} assigned clusters, {active} active-by-weight (ARI={ari:.3f})",
    )
    ...
    save_trajectory_pca(
        ...
        f"Flattened trajectory PCA colored by static DP-GMM: {count_assigned_clusters(pred)} assigned clusters (ARI={ari:.3f})",
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Dimension of the latent contractive flow before linear observation. "
            "If omitted, latent_dim = n_outputs."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.5,
        help=(
            "Condition-number-like spread of singular values in the linear "
            "observation/projection matrix. Larger values make the observed "
            "coordinates more anisotropic."
        ),
    )
    parser.add_argument(
        "--kernel-pca-gamma",
    ...
    parser.add_argument("--gamma-multiplier", type=float, default=0.1)
    ...
    parser.add_argument("--bound-sigma-high-multiplier", type=float, default=1e1)
...
def main():
    ...
    data, labels, metadata, e0, e1, start_center, W_obs = simulate_equilibrium_flows(
        seed=args.seed,
    ...
    np.save(out_dir / "linear_observation_matrix.npy", W_obs)
    ...
    print(f"  latent_dim = {args.latent_dim if args.latent_dim is not None else args.n_outputs}")
    print(f"  linear projection condition = {args.projection_condition}")
    ...
    print(
        "\nInterpretation:\n"
        "  - In comparison_results.csv, assigned_clusters is the number visible in\n"        "    plots; active_components_by_weight is the number of mixture components\n"        "    whose posterior weight exceeds the threshold. They can differ.\n"        "  - The cloud-level static DP-GMM is expected to infer multiple snapshot\n"
